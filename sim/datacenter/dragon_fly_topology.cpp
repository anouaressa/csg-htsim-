...
DragonFlyTopology::DragonFlyTopology(uint32_t p, uint32_t a, uint32_t h, mem_b queuesize, Logfile* lg, EventList* ev, queue_type q, simtime_picosec rtt) {
    ...
    _num_global_links = 2 * h;  // Double the global links compared to DragonFly
    _num_local_links = a - 1;   // Local links within group
    _congestion_threshold = 0.8; // 80% congestion threshold for adaptive routing

    _no_of_nodes = _a * _p * (_a * _h + 1);

    cout << "DragonFly Plus topology with " << _p << " hosts per router, " << _a 
         << " routers per group and " << (_a * _h + 1) << " groups, total nodes " 
         << _no_of_nodes << endl;
    cout << "Global links per router: " << _num_global_links << endl;
...
void DragonFlyTopology::update_link_congestion(uint32_t src, uint32_t dst, uint32_t queue_length) {
    auto link = make_pair(src, dst);
    auto now = _eventlist->now();
    
    LinkCongestion& cong = link_congestion[link];
    cong.queue_length = queue_length;
    cong.last_update = now;
    
    // Calculate congestion level based on queue length and capacity
    cong.congestion_level = static_cast<double>(queue_length) / _queuesize;
}

double DragonFlyTopology::get_path_congestion(const vector<pair<uint32_t, uint32_t>>& path) {
    double max_congestion = 0.0;
    
    for (const auto& link : path) {
        auto it = link_congestion.find(link);
        if (it != link_congestion.end()) {
            max_congestion = max(max_congestion, it->second.congestion_level);
        }
    }
...
vector<const Route*>* DragonFlyTopology::get_adaptive_paths(uint32_t src, uint32_t dest) {
    ...
    if (srcgroup == dstgroup) {
        // Within same group - use minimal path
        return get_bidir_paths(src, dest, false);
    }
    
    // Get all possible intermediate groups
    vector<uint32_t> intermediate_groups;
    for (uint32_t g = 0; g < _no_of_groups; g++) {
        if (g != srcgroup && g != dstgroup) {
            intermediate_groups.push_back(g);
        }
    }
    
    // For each intermediate group, find the least congested path
    for (uint32_t inter_group : intermediate_groups) {
        route_t* route = new Route();
        
        // Source to intermediate group
        uint32_t src_switch = srcgroup * _a + (inter_group % _h);
        route->push_back(queues_host_switch[src][HOST_TOR(src)]);
        route->push_back(pipes_host_switch[src][HOST_TOR(src)]);
        
        if (HOST_TOR(src) != src_switch) {
            route->push_back(queues_switch_switch[HOST_TOR(src)][src_switch]);
            route->push_back(pipes_switch_switch[HOST_TOR(src)][src_switch]);
        }
        
        // Intermediate group to destination
        uint32_t inter_switch = inter_group * _a + (dstgroup % _h);
        uint32_t dst_switch = dstgroup * _a + (inter_group % _h);
        
        route->push_back(queues_switch_switch[src_switch][inter_switch]);
        route->push_back(pipes_switch_switch[src_switch][inter_switch]);
        route->push_back(queues_switch_switch[inter_switch][dst_switch]);
        route->push_back(pipes_switch_switch[inter_switch][dst_switch]);
        
        if (dst_switch != HOST_TOR(dest)) {
            route->push_back(queues_switch_switch[dst_switch][HOST_TOR(dest)]);
            route->push_back(pipes_switch_switch[dst_switch][HOST_TOR(dest)]);
        }
        
        route->push_back(queues_switch_host[HOST_TOR(dest)][dest]);
        route->push_back(pipes_switch_host[HOST_TOR(dest)][dest]);
...
vector<const Route*>* DragonFlyTopology::get_bidir_paths(uint32_t src, uint32_t dest, bool reverse) {
    // First try adaptive routing
    vector<const Route*>* adaptive_paths = get_adaptive_paths(src, dest);
    
    // If adaptive routing found good paths, use them
    if (!adaptive_paths->empty()) {
        // Sort paths by congestion level
        sort(adaptive_paths->begin(), adaptive_paths->end(),
             [this](const Route* a, const Route* b) {
                 vector<pair<uint32_t, uint32_t>> path_a, path_b;
                 // Convert routes to link pairs
                 for (size_t i = 0; i < a->size() - 1; i += 2) {
                     path_a.push_back(make_pair(
                         find_switch((Queue*)a->at(i)),
                         find_switch((Queue*)a->at(i + 2))));
                 }
                 for (size_t i = 0; i < b->size() - 1; i += 2) {
                     path_b.push_back(make_pair(
                         find_switch((Queue*)b->at(i)),
                         find_switch((Queue*)b->at(i + 2))));
                 }
                 return get_path_congestion(path_a) < get_path_congestion(path_b);
             });
        
        // Keep only the least congested paths
        if (adaptive_paths->size() > _num_global_links) {
            adaptive_paths->resize(_num_global_links);
        }
        return adaptive_paths;
    }
    
    // Fallback to original minimal routing if adaptive routing fails
    vector<const Route*>* paths = new vector<const Route*>();
