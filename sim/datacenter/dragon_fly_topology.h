...
// Structure to track link congestion
...
public:
    ...
    // DragonFly Plus specific members
    map<pair<uint32_t, uint32_t>, LinkCongestion> link_congestion;
    uint32_t _num_global_links;  // Number of global links per router
    uint32_t _num_local_links;   // Number of local links per router
    double _congestion_threshold;  // Threshold for adaptive routing
    ...
    // DragonFly Plus specific methods
    void update_link_congestion(uint32_t src, uint32_t dst, uint32_t queue_length);
    double get_path_congestion(const vector<pair<uint32_t, uint32_t>>& path);
    vector<const Route*>* get_adaptive_paths(uint32_t src, uint32_t dest);
